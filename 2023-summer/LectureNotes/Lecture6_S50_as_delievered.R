...
# Pseudocode format: 
# r = covariance / ( s1 x s2 )

# Research question: What is the correlation between screentime and work?  


r <- covariance / (screentime.s * work.s)

# A small but meaningful inverse relationship
# inverse, or negative linear relationship

# Unstandardized covariance is: 
covariance

# Imagine, what would happen if you compute the covariance of the 
# standardized screentime and work hour values????

standardized.covariance <- sum(standardized.screentime*standardized.work) / df

# Lesson of the day: Correlation is "simply" standardized covariance
# Second lesson: Standardizing the data does not change the relationship

# Super fast way: 

cor(class.data$hrs.screentime.weekly,class.data$hrs.work.weekly)

cor(class.data)

# Scatterplot of a single correlation
par(mfrow=c(1,1))
plot(class.data$hrs.screentime.weekly,class.data$hrs.work.weekly)

# Remove the outliers on the screentime measure

filtered.class.data <- class.data[class.data$hrs.screentime.weekly < 60,]

plot(filtered.class.data$hrs.screentime.weekly,filtered.class.data$hrs.work.weekly)

cor(filtered.class.data$hrs.screentime.weekly,filtered.class.data$hrs.work.weekly)

# Lesson 3 of the day: outliers sway the relationship hugely!

boxplot(class.data$hrs.screentime.weekly)
