...
variances <- c(var(present.classes$hrs.screentime.weekly), var(past.classes$hrs.exercise.weekly))

F_max <- max(variances) / min(variances)

# This exceeds the threshold of 3.0. The variances are heterogenous. 
...
midpoints <- barplot(means, 
                     ylim = c(0,40), 
                     names.arg = c("2023 Summer","All Past"))

lower_bounds <- means - sem
upper_bounds <- means + sem

## Error bars pre-requisites
# 1. midpoint (middle of the bar plot coordinates on the x-axis)
# 2. a vector of means
